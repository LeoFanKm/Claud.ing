...
- i18n 11语言：en|es|hi|fr|ja|ko|de|pt|it|zh-CN|zh-TW
- 图片用 R2，禁止 Base64
- Claude Code 沟通语言：中文
...
### PROACTIVE Agents（自动激活）

以下 Agent 会在特定场景自动激活：

@.claude/agents/i18n-enforcer.md
@.claude/agents/screenplay-enforcer.md

| Agent               | 触发时机                     | 优先级      |
| ------------------- | ---------------------------- | ----------- |
| i18n-enforcer       | UI 文本、字符串字面量        | 🔴 CRITICAL  |
| screenplay-enforcer | ProseMirror schema、剧本元素 | 🟡 IMPORTANT |

**冲突解决**：按优先级顺序执行，CRITICAL 问题阻止提交。

### MCP 工具选择

| 场景              | 推荐 MCP   | 备选      |
| ----------------- | ---------- | --------- |
| 代码导航/符号操作 | Serena     | Grep      |
| 官方文档查询      | Context7   | WebSearch |
| 复杂调试/架构分析 | Sequential | 原生推理  |
| UI 组件生成       | Magic      | 手动编码  |
| 批量代码编辑      | Morphllm   | MultiEdit |

### Skills 使用指南

**认证**：

- `clerk-auth` - Clerk 认证流程

**项目管理**：

- `project-workflow` - 项目工作流
- `project-session-management` - 会话管理
...
1. 检查 `.claude/state/todo-state.json` 是否有未完成任务
2. 加载项目上下文和之前的决策
3. 显示待办事项提醒
